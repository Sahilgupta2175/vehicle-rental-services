...
const BookingDetails = () => {
  ...
  const [declaredLateTimeInput, setDeclaredLateTimeInput] = useState("1h");
  const [extensionReason, setExtensionReason] = useState("");
  const [submittingLateRequest, setSubmittingLateRequest] = useState(false);

  const parseLateTimeToMinutes = (value) => {
  ...
  const canCancelBooking = (booking) => {
    ...
    const thirtyMinutesBefore = new Date(
      rentalStart.getTime() - 30 * 60 * 1000,
    );
  ...
  const handleCancelBooking = async () => {
    if (
      window.confirm(
        "Are you sure you want to cancel this booking? A refund will be processed and credited within 5-7 business days.",
      )
    ) {
      try {
        ...
        if (data.refund?.success) {
          toast.success(
            "Booking cancelled successfully! Refund has been initiated and will be credited within 5-7 business days.",
          );
        } else {
  ...
  const canRequestLateExtension = () => {
    if (!booking) return false;
    if (booking.status !== "paid") return false;
    if (booking.pickup?.pickedUp) return false;
    if (booking.pickup?.extensionRequested) return false;
  ...
  const handleLateExtensionRequest = async () => {
    const lateMinutes = parseLateTimeToMinutes(declaredLateTimeInput);

    if (Number.isNaN(lateMinutes) || lateMinutes <= 30) {
      toast.error("Use format like 45, 45m, 1h, or 1h 30m (must be > 30 min)");
      return;
    }
    ...
    try {
      ...
      const { data } = await bookingApi.requestLatePickupExtension({
        bookingId,
        declaredLateTime: lateMinutes,
        extensionReason: extensionReason.trim(),
      });

      toast.success(data?.message || "Late pickup extension approved");
      await fetchBooking();
    } catch (err) {
  ...
  const handleConfirmPickup = async () => {
    if (!window.confirm("Confirm that you have picked up this vehicle?")) {
      return;
    }

    try {
      const { data } = await bookingApi.confirmPickup({ bookingId });
      toast.success(data?.message || "Vehicle pickup confirmed successfully");
      await fetchBooking();
    } catch (err) {
  ...
  const extensionAllowed = canRequestLateExtension();
  const pickupConfirmAllowed = canConfirmPickup();
      ...
      <div id="printable-receipt" className="hidden">
        ...
        <div className="print-header">
          <div>
            ...
            <div className="print-company-tagline">
        ...
        <div className="print-section">
          ...
          <div className="print-instructions">
            <ul>
              ...
              <li>
                Late charges: Rs.{" "}
                {Math.round(booking.vehicle?.pricePerHour * 1.5)}/hour after 1
                hour grace period
              </li>
        ...
        <div className="print-terms">
          ...
          <ol>
            <li>
              Renter must be at least 21 years old with valid driving license.
            </li>
            <li>
              Security deposit refunded within 7 working days after return.
            </li>
            ...
            <li>
              Company reserves right to refuse service or terminate rental.
            </li>
        ...
        <div className="print-signature">
          <div className="print-signature-box">Customer Signature</div>
          <div className="print-signature-box">Authorized Signature</div>
        </div>
        ...
        <div className="print-footer">
          ...
          >
            © {new Date().getFullYear()} Vehicle Rental Services. All rights
            reserved.
          </div>
        </div>
      </div>
            ...
            <div className="bg-linear-to-r from-emerald-500/20 to-emerald-600/20 border border-emerald-500/50 rounded-2xl p-6 flex items-center gap-4 animate-fadeIn">
              ...
              <div className="flex-1">
                ...
                <p className="text-emerald-200">
                  Your booking has been confirmed. We've sent the details to
                  your email.
                </p>
              </div>
            </div>
          ...
          <div className="bg-linear-to-br from-slate-800 to-slate-700 border border-slate-600 rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="bg-linear-to-r from-blue-600/20 to-purple-600/20 border-b border-slate-700 p-6">
              <div className="flex items-start justify-between">
                ...
                <span
                  className={`px-4 py-2 rounded-xl text-sm font-semibold border ${statusConfig.bg} ${statusConfig.text} ${statusConfig.border}`}
                >
                  ...
                  <div className="space-y-3">
                    ...
                    {booking.vehicle?.location && (
                      <div className="flex items-center gap-2 text-sm text-slate-400">
                        ...
                        {booking.vehicle.location.address},{" "}
                        {booking.vehicle.location.city}
                      </div>
                    )}
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-lg font-semibold text-amber-400">
                        ₹{booking.vehicle?.pricePerHour}/hr
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700">
                  ...
                  {booking.pickup?.extensionRequested ? (
                    <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4 text-amber-200 text-sm">
                      Extension already requested for this booking.
                      <div className="mt-2 text-amber-300/90">
                        Declared late time:{" "}
                        <strong>
                          {booking.pickup?.userDeclaredLateTime ||
                            booking.pickup?.finalWaitingWindow}{" "}
                          min
                        </strong>
                      </div>
                      {booking.pickup?.extensionReason && (
                        <div className="mt-2 text-amber-100">
                          Reason: <strong>{booking.pickup.extensionReason}</strong>
                        </div>
                      )}
                    </div>
                  ) : extensionAllowed ? (
                    <div className="space-y-4">
                      <p className="text-sm text-slate-300">
                        If you will be late, enter expected delay in
                        hours/minutes (must be above 30 minutes).
                      </p>
                      <div className="flex flex-col gap-3">
                        <input
                          type="text"
                          value={declaredLateTimeInput}
                          onChange={(e) =>
                            setDeclaredLateTimeInput(e.target.value)
                          }
                          className="w-full px-4 py-2.5 rounded-lg bg-slate-900 border border-slate-600 text-white focus:outline-none focus:ring-2 focus:ring-amber-500"
                          placeholder="e.g. 45m, 1h, 1h 30m"
                        />
                        <textarea
                          ...
                          rows={3}
                          className="w-full px-4 py-2.5 rounded-lg bg-slate-900 border border-slate-600 text-white focus:outline-none focus:ring-2 focus:ring-amber-500"
                          placeholder="Reason for being late (e.g., traffic jam, medical emergency, train delay)"
                        />
                        <button
                          onClick={handleLateExtensionRequest}
                          disabled={submittingLateRequest}
                          className="px-5 py-2.5 rounded-lg bg-amber-600 hover:bg-amber-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-medium transition-all"
                        >
                          {submittingLateRequest
                            ? "Submitting..."
                            : "Request Extension"}
                        </button>
                      </div>
                      <p className="text-xs text-slate-400">
                        Accepted formats: <strong>45</strong>,{" "}
                        <strong>45m</strong>, <strong>1h</strong>,{" "}
                        <strong>1h 30m</strong>
                      </p>
                    </div>
                  ) : (
                    <div className="bg-slate-900/60 border border-slate-700 rounded-lg p-4 text-slate-300 text-sm">
                      Extension window is not available for this booking right
                      now.
                    </div>
                  )}

                  {pickupConfirmAllowed && (
                    <div className="mt-4 pt-4 border-t border-slate-700">
                      <button
                        onClick={handleConfirmPickup}
                        className="px-5 py-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-medium transition-all flex items-center gap-2"
                      >
                        <svg
                          className="w-4 h-4"
                          fill="none"
