...
const UserDashboard = () => {
  ...
  const handleCancelBooking = async (bookingId) => {
    if (
      !window.confirm(
        "Are you sure you want to cancel this booking? If you have already paid, a refund will be processed.",
      )
    ) {
      return;
    }
  ...
  const canRequestLateExtension = (booking) => {
    ...
    const extensionDeadline = new Date(
      bookingStart.getTime() + defaultWaitingWindow * 60 * 1000,
    );

    return now <= extensionDeadline;
  };

  const handleRequestLateExtension = async (booking) => {
    const input = window.prompt(
      "Enter expected delay (minutes or hours). Examples: 45, 45m, 1h, 1h 30m",
      "1h",
    );

    if (input === null) return;

    const declaredLateTime = parseLateTimeToMinutes(input);
    if (Number.isNaN(declaredLateTime)) {
      toast.error("Invalid format. Use 45, 45m, 1h, or 1h 30m");
      return;
    }

    if (declaredLateTime <= 30) {
      toast.error("Please enter more than 30 minutes (e.g., 40 or 60)");
      return;
    }

    const extensionReason = window.prompt(
      "Enter reason for late pickup (required)",
      "Traffic delay",
    );

    if (extensionReason === null) return;
  ...
  const canConfirmPickup = (booking) => {
    if (booking.status !== "paid") return false;
    if (booking.pickup?.pickedUp) return false;
    return true;
  };
  ...
  const handleConfirmPickup = async (booking) => {
    ...
    try {
      const { data } = await bookingApi.confirmPickup({
        bookingId: booking._id,
      });
      toast.success(data?.message || "Pickup confirmed successfully");
      await loadBookings();
    } catch (err) {
        ...
        <div className="relative overflow-hidden bg-linear-to-br from-slate-800 via-slate-800 to-blue-900/30 border border-slate-600/50 rounded-3xl p-8 shadow-2xl">
            ...
            <div className="flex items-start gap-5">
              ...
              <div>
                ...
                <p className="text-slate-400 text-base">
                  Track and manage all your vehicle reservations in one place
                </p>
              </div>
            </div>
            <button
          ...
          <div className="relative grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8">
            {[
              ...
              <div
                key={stat.label}
                className={`bg-linear-to-br ${stat.gradient} border ${stat.border} rounded-2xl p-5 hover:scale-105 transition-all duration-300 backdrop-blur-sm`}
              >
                ...
                <BookingCard
                  ...
                  actions={
                    <div className="flex gap-2">
                      ...
                      {canRequestLateExtension(b) && (
                        <button
                          onClick={() => handleRequestLateExtension(b)}
                          className="px-4 py-2 rounded-lg bg-amber-600 hover:bg-amber-700 text-white text-sm font-medium transition-all flex items-center gap-2 shadow-lg shadow-amber-500/30"
                          title="Request late pickup extension"
                        >
                          <svg
                            className="w-4 h-4"
                            fill="none"
                      ...
                      {canConfirmPickup(b) && (
                        <button
                          ...
                          className="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium transition-all flex items-center gap-2 shadow-lg shadow-emerald-500/30"
                      ...
                      {["approved", "paid"].includes(b.status) && (
                        <button
                          onClick={() =>
                            canCancelBooking(b) && handleCancelBooking(b._id)
                          }
                          disabled={!canCancelBooking(b)}
                          title={
                            !canCancelBooking(b)
                              ? "Cannot cancel within 30 minutes of rental start time"
                              : "Cancel this booking"
                          }
                          className={`px-4 py-2 rounded-lg text-white text-sm font-medium transition-all flex items-center gap-2 shadow-lg ${
                            canCancelBooking(b)
                              ? "bg-red-600 hover:bg-red-700 shadow-red-500/30 cursor-pointer"
